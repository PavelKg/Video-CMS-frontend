<template>
  <div class="table-messages">
    <table class="tb-messages">
      <thead>
        <th v-for="(column, index) in columns_box" :key="index">{{column}}</th>
      </thead>
      <tbody>
        <tr v-for="message in message_list" :key="message.id">
          <td align="center">
            <b-form-checkbox :id="String(message.id)" :name="`ch-${message.id}`"></b-form-checkbox>
          </td>
          <td>
            <a href="#" @click="showMessageModal(message.id)">{{message.subject}}</a>
          </td>
          <td>{{message.user}}</td>
    ...
    <ModalMessage v-if="onShowModalMessageInfo" @close="onShowModalMessageInfo = false"/>
  </div>
</template>
